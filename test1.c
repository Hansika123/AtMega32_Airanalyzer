...
void displayco(){
	...
	for(i=0;i<3;i++){ // i for display timing
		...
		convertstr(n2);  //add 100 string value to str
		Lcd4_Write_String(str);
		Lcd4_Write_String("%");
...
//---------------o2 calculation
	...
	val = (d1-ReadADC(2));  //d1 dust 1
	if (val<0){
		val=0;  //callibarate
	}
	dust1=val;
	itoa(dust1,valds1,10);
	char ds1[]=" D1:";
	strcat(ds1,valds1);
	strcat(airdata,ds1);
	...
	val = (d2-ReadADC(3)); //2 dust 2
	if (val<0){
		val=0;  //callibarate
	}
	dust2=val;
	itoa(dust2,valds2,10);
	char ds2[]=" D2:";
	strcat(ds2,valds2);
	strcat(airdata,ds2);
...
void transmeet2(){
	...
	char valaq[3], vallpg[3];
	char airdata[55]="Airdata::";
	char linebreak[]="\n\r";


	val=(1024-ReadADC(5)); // adc  pin connect n2
	val += 50;
...
//	val=((float)val/1023)*100;
	...
	val=ReadADC(7);   // adc 7 pin connect mq2 lpg
	val -=240;
...
USARTInit(12);
	...
	while(1){
		//BtString("Airdata:: AQ:89 LPG:01 CO:00 D1:122 D2:211  co sd cds sdad \n\r");
		transmeet();
		onled();
